...
class TransformPathAliases {
  ...
  _getAliasMapFromTsConfig(projectRoot) {
    ...
    if (!fs.existsSync(tsconfigPath)) {
      throw new Error('tsconfig.json not found. Path aliases require a valid tsconfig.json file.')
    }
    ...
    try {
      ...
      if (!pathsMatch) {
        throw new Error(
          'No paths section found in tsconfig.json. Path aliases require a paths configuration.'
        )
      }
      ...
      if (Object.keys(aliasMap).length === 0) {
        throw new Error(
          'No valid path aliases found in tsconfig.json. At least one path mapping is required.'
        )
      }
